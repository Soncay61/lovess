...
let isFirstClick = true; // İlk tıklama kontrolü için bayrak

yesButton.addEventListener('click', function () {
    if (isFirstClick) {
        const audio = new Audio('ses/sess.mp3');
        audio.play();
        isFirstClick = false; // İlk tıklama gerçekleştiğinde bayrağı false yap
    }

    generateHearts();

    const title = document.querySelector('.title');
    title.textContent = 'Yasasıınnn';
});
...
document.addEventListener('keydown', function (event) {
    if (event.key === 'Tab') {
        noButton.style.animation = 'explode 0.2s forwards';
...
window.addEventListener("beforeunload", function (e) {
    var confirmationMessage = "Dur Nereye Gideysun?";
